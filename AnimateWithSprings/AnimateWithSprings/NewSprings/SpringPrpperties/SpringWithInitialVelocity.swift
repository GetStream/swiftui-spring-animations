//
//  SpringWithInitialVelocity.swift
//  AnimateWithSprings
//
...
struct SpringWithInitialVelocity: View {
    // Animation phases
    ...
    var body: some View {
        ...
        return PhaseAnimator(AnimationPhase.allCases) { move in
            ZStack {
                Divider()
                Circle()
                    .trim(from: 0.5)
                ...
                Circle()
                    ...
                    .offset(x: -180)
                    .rotationEffect(move == .zero ? .degrees(0) : .degrees(180))
            }
            .padding()
        } animation: { move in
